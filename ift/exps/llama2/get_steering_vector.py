...
def generate(data, config, pos_or_neg):
    ...
    """
    model_name_or_path = (
        config["model"] if pos_or_neg == "negative" else config["ift_model_path"]
    )
    model, tokenizer = get_model_and_tokenizer(model_name_or_path, config["tokenizer"])
...
def generate_series(data, pos, config):
    ...
    """
    model, tokenizer = get_model_and_tokenizer(config["model"], config["tokenizer"])
        ...
        for timestep in range(steer_timesteps):
            ...
            with torch.inference_mode():
                output_bs = model.generate(
                    ...
                    attention_mask=(prompt_w_ift_bs != tokenizer.pad_token_id).to(
                        model.device
                    ),
                    do_sample=False,
                    max_new_tokens=1,
...
def make_pos_neg_pair(data, config, cache_dir=None):
    """
    ...
    if cache_dir is not None:
        ...
        torch.save(pos_toks_Ds, pos_sample_filepath)
    ...
    if cache_dir is not None:
        contrast_filepath = os.path.join(cache_dir, "pos_neg_pairs.pt")
        torch.save(contrast_data, contrast_filepath)
...
def get_steer_vector(data, config):
    ...
    contrast_data = load_pos_neg_pair_from_file(
        os.path.join(config["cache_dir"], "pos_neg_pairs.pt")
    )

    llama2, tokenizer = get_model_and_tokenizer(config["model"], config["tokenizer"])
        ...
        for idx in tqdm(range(0, len(data["prompts"]), batch_size)):
            ...
            with record_activations(
                ...
                acts_bld = torch.stack(
                    [record[layer][0][:, -1, :].cpu() for layer in range(24)],  # TODO
                    dim=1,
                )
...
def main():
    ...
    # Positive
    #train_data["prompts"] = train_data["prompts"][: config["batch_size"]]
    # data_size = len(train_data["prompts"])
    #print("Generating positive samples.")

    cache_dir = config["cache_dir"]
    #pos_sample_filepath = os.path.join(cache_dir, "pos_generations.pt")

    #pos_toks_Ds = generate(train_data, config, "positive").cpu()
    #torch.save(pos_toks_Ds, pos_sample_filepath)

    #pos_toks_Ds = torch.load(pos_sample_filepath)
    #print("Generating negative samples.")
    #neg_toks_Dts = generate_series(train_data, pos_toks_Ds, config)

    #neg_sample_filepath = os.path.join(cache_dir, "neg_generations.pt")
    #torch.save(neg_toks_Dts, neg_sample_filepath)
    ...
    save_steer_vecs(steer_vecs, "steer_vec_series.pt")
